...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12 bg-grid">
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[radial-gradient(circle,rgba(59,130,246,0.06),transparent_60%)] pointer-events-none" />
      ...
      >
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[var(--color-blue)] to-[var(--color-violet)] flex items-center justify-center mx-auto mb-4 animate-pulse-glow">
            <Eye size={28} className="text-white" />
          </div>
          <h1 className="text-2xl font-bold mb-1">Create Account</h1>
          <p className="text-sm text-[var(--color-text-secondary)]">Join RetiNex AI Platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm text-[var(--color-text-secondary)] mb-2 font-medium">Username</label>
            <div className="relative">
              <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" />
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="input-field !pl-11" placeholder="your_username" required />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field !pl-11" placeholder="Minimum 6 characters" required />
            </div>
          </div>
        ...
        <p className="text-center text-sm text-[var(--color-text-muted)] mt-6">
          ...
          <Link to="/login" className="text-[var(--color-teal)] hover:underline no-underline font-medium">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
